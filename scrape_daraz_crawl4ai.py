...
def normalize_price(s: str | None) -> str | None:
    if not s:
        return None
    # Specifically look for Bengali currency (৳) or Tk followed by numbers
...
async def main():
    ...
    scroll_rounds = 6         # try loading more products
    ...
    products2_collection = db["products2"]  # New collection

    # Test connection with both collections
    products2_collection.insert_one({"test": "connection"})
    print("MongoDB test documents:")
    print("- products2:", products2_collection.find_one({"test": "connection"}))
    ...
    async with AsyncWebCrawler(verbose=True) as crawler:
        for page_num in range(1, max_pages + 1):
            ...
            try:
                result = await crawler.arun(
                    ...
                    js_code=f"""
                        ...
                        containers.forEach(card => {{
                            ...
                            const original = getOriginalFrom(card);
                ...
                # If JS didn’t capture, attempt robust soup parse (rendered HTML)
                if not page_products:
                    ...
                    # keep is_last_page True unless we detect otherwise
    ...
    if final_products:
        try:
            # Insert into products2 collection
